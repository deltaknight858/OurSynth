...
const HaloCheckbox = React.forwardRef<HTMLInputElement, HaloCheckboxProps>(
  function HaloCheckbox({ label, description, indeterminate, size = "md", variant = "primary", error = false, className, ...props }, ref) {
    return (
      <label className={cn("flex items-center gap-2 cursor-pointer select-none", className)}>
        ...
        {description && <span className="text-xs text-[rgb(var(--halo-muted))] ml-2">{description}</span>}
      </label>
    );
  }
);
