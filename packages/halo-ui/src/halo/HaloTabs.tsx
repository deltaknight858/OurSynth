...
export default function HaloTabs({
  ...
  const variantClasses = {
    glass: {
      ...
      activeTab:
        'bg-[rgba(var(--halo-primary),0.15)] text-[rgb(var(--halo-primary))] border-b-2 border-[rgb(var(--halo-primary))]',
      inactiveTab:
        'text-[rgb(var(--halo-muted))] hover:text-[rgb(var(--halo-fg))] hover:bg-[rgba(var(--halo-fg),0.05)]',
    },
    elevated: {
      container:
        'bg-[rgb(var(--halo-bg-elev))] rounded-halo shadow-[var(--halo-shadow)] border border-[rgba(var(--halo-fg),0.1)]',
      tabList: 'border-b border-[rgba(var(--halo-fg),0.1)]',
      activeTab:
        'bg-[rgba(var(--halo-primary),0.1)] text-[rgb(var(--halo-primary))] border-b-2 border-[rgb(var(--halo-primary))]',
      inactiveTab:
        'text-[rgb(var(--halo-muted))] hover:text-[rgb(var(--halo-fg))] hover:bg-[rgba(var(--halo-fg),0.05)]',
    },
    minimal: {
  ...
  return (
    <div className={cn('w-full', variantClasses[variant].container, className)}>
      {/* Tab List */}
      <div className={cn('flex', variantClasses[variant].tabList)} role="tablist">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            className={cn(
      ...
      {/* Tab Content */}
      <div className={cn(sizeClasses[size].content)}>{currentTab?.content}</div>
    </div>
  );
}
