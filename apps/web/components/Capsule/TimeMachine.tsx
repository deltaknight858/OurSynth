  ...
  return (
    <div className="glass-panel p-6 rounded-xl shadow-neon">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={() => setStep(Math.min(events.length - 1, step + 1))} disabled={step === events.length - 1}>→</button>
      </div>
      <div className="mt-4">
        <button className="neon-button" onClick={() => alert('Replay not implemented yet!')}>Replay This State</button>
        <button className="neon-button ml-2" onClick={() => alert('Export coming soon!')}>Export Capsule</button>
      </div>
    </div>
  );
}