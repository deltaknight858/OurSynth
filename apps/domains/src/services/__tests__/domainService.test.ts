...
describe("domainService", () => {
  ...
  describe("purchaseDomain", () => {
    it("should throw an error if user is not authenticated", async () => {
      ...
      await expect(domainService.purchaseDomain("new.com", 1)).rejects.toThrow(
  ...
  describe("renewDomain", () => {
    it("should renew a domain successfully", async () => {
      ...
      const fetchMock = {
        ...
        single: jest.fn().mockResolvedValue({ data: existingDomain, error: null }),
      };
      const updateMock = {
        update: jest.fn().mockReturnThis(),
        eq: jest.fn().mockReturnThis(),
        select: jest.fn().mockReturnThis(),
        single: jest.fn().mockResolvedValue({ data: updatedDomain, error: null }),
      };
  ...
  describe("deleteDomain", () => {
    it("should delete a domain successfully", async () => {
      ...
      const fromMock = {
        ...
        eq: jest.fn().mockReturnThis(),
        select: jest.fn().mockReturnThis(),
        single: jest.fn().mockResolvedValue({ data: deletedDomain, error: null }),
      };
      (mockSupabase.from as jest.Mock).mockReturnValue(fromMock);
