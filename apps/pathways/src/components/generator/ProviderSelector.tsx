    ...
    <Box sx={{ mb: 3 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'flex-start' }}>
        ...
        <Tooltip title="View provider information and setup instructions">
          <IconButton size="small" onClick={() => window.open('/docs/ai-providers', '_blank')}>
            <InfoIcon />
          </IconButton>
        </Tooltip>
      </Box>
      ...
      {selectedProviderConfig && (
        <Alert severity={providerStatus[selectedProvider] ? 'success' : 'warning'} sx={{ mt: 2 }}>
          <Typography variant="body2">
            <strong>{selectedProviderConfig.name}</strong>:{' '}
            {getProviderStatusText(selectedProviderConfig)}
          </Typography>
          {!providerStatus[selectedProvider] && (
            <Typography variant="caption" display="block" sx={{ mt: 1 }}>
              Add the required environment variables to your .env.local file to enable this
              provider.
            </Typography>
          )}
        </Alert>
