  ...
  const renderComponentPreview = () => {
    ...
    try {
      ...
      const hasReactComponent =
        /(?:const|function)\s+[A-Z]\w*.*(?:React\.FC|FunctionComponent|\(\s*\)\s*=>|\s*=\s*\(\s*\)\s*=>)/.test(
          code,
        );
      ...
      if (!hasReactComponent && !hasJSXReturn && !hasJSXElements) {
        return (
          <Alert severity="info">
            <Typography variant="body2">
              This code doesn't appear to be a React component. Preview is only available for React
              components.
            </Typography>
          </Alert>
        );
      ...
      const uiElements = {
        ...
        textFields:
          code.match(/TextField[^>]*(?:variant=["']([^"']*)|placeholder=["']([^"']*))?[^>]*>/g) ||
          [],
        cards: code.match(/Card[^>]*>/g) || [],
        chips: code.match(/Chip[^>]*(?:label=["']([^"']*)|color=["']([^"']*))?[^>]*>/g) || [],
        typography:
          code.match(
            /Typography[^>]*(?:variant=["']([^"']*)|color=["']([^"']*))?[^>]*>([^<]*)</g,
          ) || [],
      ...
      const complexityScore = Object.values(uiElements).reduce(
      ...
      return (
        <Alert severity="warning">
          <Typography variant="body2">
            Unable to analyze component structure:{' '}
  ...
  const renderLivePreview = () => {
    ...
    return (
      <Box sx={{ p: 3 }}>
        ...
        <Box key={`preview-${codeHash}-${codeStats.lines}`}>{renderComponentPreview()}</Box>
        ...
        <Box sx={{ mt: 3, p: 2, bgcolor: 'background.default', borderRadius: 1 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            In a production environment, this preview would use a secure iframe sandbox or
            server-side rendering to safely execute and display the component.
          </Typography>
        </Box>
      </Box>
    );
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Button variant="contained" startIcon={<Save />} onClick={handleSaveOpen} size="small">
            Save
          </Button>
      ...
      <Box sx={{ mb: 2 }}>
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
          <Card
            variant="outlined"
            sx={{ textAlign: 'center', p: 1, flex: '1 1 auto', minWidth: 100 }}
          >
            <Typography variant="caption" color="text.secondary">
              Lines
            </Typography>
      ...
      {/* Save Dialog */}
      <Dialog open={saveDialogOpen} onClose={handleSaveClose} maxWidth="sm" fullWidth>
