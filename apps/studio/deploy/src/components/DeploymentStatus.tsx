...
export function DeploymentStatus({ deployments }: DeploymentStatusProps) {
  return (
    <motion.div
      ...
      className="mt-8"
    >
      <h2 className="text-xl font-semibold text-neon mb-4">Recent Deployments</h2>
          ...
          deployments.map((deployment) => (
            ...
            >
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-medium text-white">{deployment.projectName}</h3>
...
function StatusBadge({ status }: { status: 'running' | 'completed' | 'failed' }) {
  ...
  return (
    <span className={`${config.color} bg-opacity-20 text-xs font-medium px-2.5 py-0.5 rounded-full border border-current`}>
      {config.text}
    </span>
  );
}