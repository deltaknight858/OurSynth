    ...
    <div className="h-full flex flex-col gap-2 p-4 bg-black/20 rounded-xl">
      ...
      <HaloScrollArea className="flex-1 space-y-2">
        {timeline.map((evt) => (
          <div
            key={evt.id}
            className={`p-2 rounded cursor-pointer ${selected === evt.id ? 'bg-cyan-900/40' : 'bg-cyan-800/10'}`}
            onClick={() => setSelected(evt.id)}
          >
            <div className="text-xs text-cyan-200">
              [{new Date(evt.ts).toLocaleTimeString()}] {evt.type}
            </div>
            <div className="text-xs text-white/80">{JSON.stringify(evt.data)}</div>
            {evt.data &&
              typeof evt.data === 'object' &&
              'meshSimResult' in evt.data &&
              typeof (evt.data as any).meshSimResult === 'object' &&
              (evt.data as any).meshSimResult.simId && (
                <button
                  className="ml-2 px-2 py-1 rounded bg-cyan-700/40 text-cyan-100 text-xs font-bold"
                  onClick={(e) => {
                    e.stopPropagation();
                    showMeshSimEvidence((evt.data as any).meshSimResult.simId);
                  }}
                >
                  Simulation Evidence
                </button>
            ...
            <button
              ...
              onClick={() => promoteToDeploy(evt.id)}
            >
              Promote to Deploy
            </button>
          </div>
