...
test('MeshSim provenance artifact appears in timeline and can be replayed', async ({ page }) => {
  ...
  await page.evaluate(() => {
    window.dispatchEvent(new CustomEvent('oai:event', {
      detail: {
        id: 'sim-123',
        ts: Date.now(),
        source: 'deploy',
        ...
        data: {
          meshSimResult: {
            ...
            nodeReports: [
              { nodeId: 'n1', phases: [{ name: 'start', start: 't1', end: 't2', status: 'ok' }], signatureVerified: true },
              { nodeId: 'n2', phases: [{ name: 'start', start: 't1', end: 't2', status: 'ok' }], signatureVerified: true },
              { nodeId: 'n3', phases: [{ name: 'start', start: 't1', end: 't2', status: 'ok' }], signatureVerified: true }
            ],
            latencyMatrix: [[0,12,15],[12,0,18],[15,18,0]],
            outcome: 'pass',
