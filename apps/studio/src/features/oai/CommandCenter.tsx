...
export function OAICommandCenter() {
  ...
  useEffect(() => {
    fetch('/api/wizard/actions').then(r => r.json()).then(setActions);
  }, []);
      ...
      <HaloCard className="shadow-2xl bg-black/90 border-cyan-400/40">
        ...
        <div className="mb-2">
          <div className="flex gap-2 overflow-x-auto pb-2">
            {actions.map(a => (
              <HaloButton key={a.id} label={a.id} onClick={() => handleSelect(a)} variant={selected?.id===a.id?'primary':'secondary'} />
            ))}
          </div>
        </div>
        {selected && (
          <div className="mb-2">
            ...
            {selected.params.map(p => (
              <input key={p} className="w-full mb-1 px-2 py-1 rounded bg-cyan-950/40 text-cyan-100" placeholder={p} value={params[p]||''} onChange={e=>handleParamChange(p,e.target.value)} />
            ))}
        ...
        {job && (
          <div className="mb-2 text-xs">
            <div>Status: <span className={job.status==='completed'?'text-green-400':job.status==='failed'?'text-red-400':'text-yellow-300'}>{job.status}</span></div>
            <div>Artifacts: {job.artifacts?.map(a => <span key={a.artifactId} className="block text-cyan-200">{a.artifactId}</span>)}</div>
          </div>
        )}
        {logs && (
          <pre className="bg-black/60 rounded p-2 text-xs text-cyan-200 max-h-40 overflow-y-auto">{logs}</pre>
        )}
      </HaloCard>
    </div>
  );
